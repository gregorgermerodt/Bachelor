...
public class Matrix {
	...
	public int[] getColumnVector(int col) {
		...
		int[] ausgabe = new int[values.length];
		
		for(int i=0;i<values.length;i++){
			ausgabe[i] = values[i][col];
		}
	...
	public void transpose() {
		int[][] hilfvalues = new int[values[0].length][values.length];
		
	for(int i=0;i<values.length;i++){
			
			for(int j=0;j<values[0].length;j++){
				
				hilfvalues[j][i] = values[i][j];
				
			}
		}
	
	
	values = hilfvalues.clone();
	}
