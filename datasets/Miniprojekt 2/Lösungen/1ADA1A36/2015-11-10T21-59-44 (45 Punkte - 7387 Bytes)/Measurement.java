...
public class Measurement 
{
	...
	public void addValue(int value) 
	{
		...
		{
			tempdata[tempdata.length-1-b]=data[data.length-1-b];
			b++;
		}
		data=tempdata;
	...
	{		
		int[] vardata;		// für die Länge
		vardata= new int[0];
		
		int[] tempdata;
		tempdata= new int[vardata.length];
		
		int k=0;
		int j=0;
		
		for(int i=0; i<=data.length; i++) // bis ende data array
		{
			if(data[j]>threshold)
			{
				vardata=new int[vardata.length+1];
				tempdata[k]=threshold;
				k++;
				j++;
			}
			else
