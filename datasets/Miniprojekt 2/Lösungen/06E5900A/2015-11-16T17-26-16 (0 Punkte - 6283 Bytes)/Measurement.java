...
public class Measurement {
	...
	public void addValue(int value) {
		int [] groeßererArray = new int [data.length+1];
		for(int index =0; index <data.length;index++)
		{
			groeßererArray[index] = data[index];
		}
		groeßererArray [groeßererArray.length-1]=value;
		data = groeßererArray;
		}
	...
	public void addValues(int[] values) {
		...
		for(int neuerindex =0; neuerindex <values.length;neuerindex++){
			biggerArray[speicher]= values[neuerindex];
			speicher++;
		}
		data=biggerArray;
	}
