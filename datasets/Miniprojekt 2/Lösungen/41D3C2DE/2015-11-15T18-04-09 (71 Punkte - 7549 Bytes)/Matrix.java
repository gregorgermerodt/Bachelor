...
public class Matrix {
	...
	public void add(Matrix m) {
		int[][] result= new int[m.values.length][m.values[0].length];
	...
	public boolean isEqualTo(Matrix m) {
		...
		for(int i=0; i<values.length; i++){
			for(int j=0; j<values[0].length;j++){
	if(values[i][j] !=m.values[i][j] && values.length==m.values.length && values[0].length ==m.values[0].length){return false;
			}			
			}
			}
		return true;
