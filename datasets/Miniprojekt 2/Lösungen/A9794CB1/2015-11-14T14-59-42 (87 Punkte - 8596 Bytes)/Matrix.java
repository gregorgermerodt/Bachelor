...
public class Matrix {
	...
	public boolean isEqualTo(Matrix m) {
		...
		for(int i=0; i<m.values.length-1; i++){
			for(int j=0; j<values[i].length; j++){
		if(Arrays.equals(m.values, values))
			return true;
			}
		}
		return false;
			...
			//for(int j=0; j<values[i].length; j++){
				//if(m.values.length == values.length && 
					//	Arrays.equals(m.values, values)
						//&& m.values[i]==values[i] && m.values[j]== values[j]
	...
	public void transpose() {
		...
		int zeilen = values.length-1;
		int spalten = values[zeilen].length;
		int[][] newArray = new int[zeilen][spalten];
