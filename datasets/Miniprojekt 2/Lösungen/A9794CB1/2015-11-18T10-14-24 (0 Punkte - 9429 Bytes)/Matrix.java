...
public class Matrix {
	...
	public void scalarMultiplication(int c) {
		...
		for(int i=0; i<values.length; i++){//Anzahl der zeilen
	...
	public int[] getColumnVector(int col) {
	 ...
	 public int getDerKleinsteIndex(int[] values){
     	return values[0];
     }
     
     public int getDerGrößteIndex(int[] values){
     	return values[values.length-1];
     }

	public boolean isEqualTo(Matrix m) {
			if (values.length == m.values.length
					&& values[0].length == m.values[0].length && Arrays.equals(m.values, values)) {
				for (int i = 0; i < values.length; i++) {
	...
	//	for(int i=0; i<m.values.length-1; i++){
			...
			//	if(m.values.length == values.length && 
						//Arrays.equals(m.values, values)&&
			//			m.values[i].length == values[i].length)
			//		return true;
		//	}
		//}
