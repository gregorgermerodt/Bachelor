...
/**2e)*/	
	...
	{
		if(this.values.length == m.values.length) //1. Zeilen sind gleich?
		{
			for(int i = 0; i< m.values.length; i++)	// 2.1 Um Array-Länge der nächst tieferen Dimenson zu prüfen ist Index der vorherigen notwendig
			{
				if(this.values[i].length == m.values[i].length) // 2.2 Spalten gleich?
				{
					for(int z = 0; z < m.values.length; z++) // 3. Inhalt identisch?
					{
						for(int s = 0; s < m.values[z].length; s++)
						{
							if(this.values[z][s] == m.values[z][s])
								return true;
						}
							return false;
					}
