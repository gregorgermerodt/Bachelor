...
public class Matrix {
	...
	public boolean isEqualTo(Matrix m) {
		...
		boolean k = false;
		int x = 0;
		int y = values.length * values[0].length;
	...
	public void transpose() {
		int[][] a = new int[values.length][values[0].length];
		for(int i = 0; i < a.length; i++){
			for(int j = 0; j < a[i].length; j++){
				a[i][j] = values[i][j];
			}
		}
		this.values = new int[a[0].length][a.length];
		for(int i = 0; i < a.length; i++){
			for(int j = 0; j < a[0].length; j++){
				this.values[j][i] = a[i][j];
			}
		}
