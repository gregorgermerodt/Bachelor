...
public class Measurement {
	...
	public int[] getValuesAboveThreshold(int threshold) {
		int[] schwellenwerte = new int [0];
		int[] neu;
		for(int index=0; index< data.length; index++)
		{
			if(data[index]>threshold)
			{
				neu= new int [schwellenwerte.length +1];
				for(int index2=0; index2< schwellenwerte.length ; index2++)
				{
					neu[index2]= schwellenwerte [index2];
					
				}
				neu[schwellenwerte.length] = data[index];
				schwellenwerte= neu ;
			}
