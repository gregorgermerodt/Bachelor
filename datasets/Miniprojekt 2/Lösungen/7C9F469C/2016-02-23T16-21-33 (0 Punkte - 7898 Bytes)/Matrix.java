...
public class Matrix {
	...
	public boolean isEqualTo(Matrix m) {
		...
		for(int j = 0; j < values.length; j++){
			for (int g = 0 ; g < values[g].length ; g++){
				if (values[j][g] != m.values[j][g]){
					return false;			
				}
			}
	}
	...
	int transe [][] = new int [values[0].length] [values.length];
	for (int i =0 ; i < values.length; i++){
		for (int j = 0; j < values[0].length; j++){
			transe[j][i] = values[i][j] ;
		}
	}
	values = new int [transe.length][transe[0].length] ;
	values = transe ;
	}
