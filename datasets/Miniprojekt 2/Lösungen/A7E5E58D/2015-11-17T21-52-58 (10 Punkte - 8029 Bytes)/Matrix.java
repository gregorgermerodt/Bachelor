...
public class Matrix {
	...
	public boolean isEqualTo(Matrix m) {
		...
		if(counter == this.values.length * m.values[0].length){
			return true;
		}else{
		return false;
		}
	...
	public void transpose() {
		...
		int[][] transpose = new int[this.values[0].length][this.values.length];
		
		for(int i = 0; i < this.values[0].length; i++){
			
			for(int j = 0; j < this.values.length; j++){
				
				transpose[i][j] = this.values[j][i];
			}
		}
		this.values = transpose;
	}
