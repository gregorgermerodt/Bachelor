...
public class Measurement {
	...
	public void addValue(int value) {
		...
		neu[neu.length -1] = value; //k�nnte auch neu[data.length] = value; schreiben 
		data = neu;
	}
	...
	public void addValues(int[] values) {
		...
		for(int index = data.length; index < data.length + values.length; index++){
		  ...
		  //values[??]; data.length=1; values.length=11; 
		  //1. Schleifendurchlauf: index - data.length = 1-1=0
		  // letzter Durchlauf; index-data.length = 11-1=0
		data = neu;
	}
