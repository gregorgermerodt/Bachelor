...
public class Matrix {
	...
	public void scalarMultiplication(int c) {
		...
		{
			for(int spalten=0; spalten<values[row].length; spalten++)
			{
				values[row][spalten]*=c;
			}
		}
	}
	...
	public void add(Matrix m) {
		for(int row = 0; row< values.length; row++)
		{
			for(int spalten=0; spalten<values[row].length; spalten++)
			{
				values[row][spalten] = values[row][spalten] + m.values[row][spalten];
			}
		}
	}

	public int[] getColumnVector(int col) {
		int[] neu = new int [this.values.length];
		for(int i=0; i<this.values.length; i++){
			neu[i] = this.values[i][col];
		}
		return neu;
	}
