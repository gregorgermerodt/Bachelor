...
public class Matrix {
	...
	public int[] getColumnVector(int col) {
		int[] neuesArray = new int [values.length];
			for(int i=0;i<values.length;i++){
				for(int j=0;j<values[i].length;j++){
					neuesArray[i] = values[i][col];
				}
			}
	...
	public boolean isEqualTo(Matrix m) {
				...
				for(int i=0;i<values.length;i++){
					for (int j=0;j<values[i].length;j++){
						if (values[i][j]!=m.values[i][j] || values.length != m.values.length || values[i].length != m.values[i].length){
							return false;
						}
					}
				}
