...
public class Matrix {
	...
	public void scalarMultiplication(int c) {
		...
		/*
		int laengeSpalteMax = values.length;
		int laengeSpalte=0;
		int laengeZeileMax = values[0].length;
		int laengeZeile = 0;
		int[][] multi = new int[laengeSpalte-1][laengeZeile-1];
		System.out.println("langeS =" + laengeSpalte);
		System.out.println("langeZ =" + laengeZeile);
		/*
		while(laengeSpalte<=laengeSpalteMax)
			while(laengeZeile<=laengeZeileMax)
				multi[laengeSpalte][laengeZeile] = values [laengeSpalte][laengeZeile]*c;
				laengeZeile++;
			laengeSpalte++;	
		---
		for(int i=0;i<=laengeSpalte;i++){
			multi[laengeSpalte][laengeZeile] = values[laengeSpalte][laengeZeile]*c;
			if(laengeZeile<=laengeZeileMax){
				laengeZeile= laengeZeile+1;
			}
			else{
				if(laengeSpalte<=laengeSpalteMax){
					laengeSpalte = laengeSpalte +1;
				}
	...
	public void add(Matrix m) {
		...
		/*
		//int[][] gib = m;
		int[][] erg = new int[values.length][values[0].length];
		for(int i=0;i<values[i].length;i++){
			int zaehl=0;
			int spalte=0;
			//erg[spalte][zaehl]=values[spalte][zaehl]+m[spalte][zaehl];
			zaehl++;
			if(i==values[i].length)
				for(int b=0;b<values[b].length;b++){
	...
	public int[] getColumnVector(int col) {
		...
		while(vector.length<values.length){
			vector[a] = values[col][a];
			a++;
		}
	...
	public boolean isEqualTo(Matrix m) {
		...
		if(values.length!=m.values.length && values.length!=m.values.length){
