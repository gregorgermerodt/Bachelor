...
public class Matrix {
	...
	public boolean isEqualTo(Matrix m) {//Matrix m stellt die Vergleichsmatrix dar.
		if(values.length == m.values.length && values[0].length == m.values[0].length){//falls
			return true;
		}
		for( int i = 0; i < values.length; i++){//durchläft die Zeilen
			for( int j = 0; j < values[i].length; j++){//durchläuft die Spalten
				if(values [i][j] != m.values[i][j]){//falls spalten und Zeilen ungleih der Spalten und Zeilen der Vergleichsmatrix 
