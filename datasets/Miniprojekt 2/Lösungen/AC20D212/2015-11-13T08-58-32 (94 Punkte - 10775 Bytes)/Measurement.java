...
public class Measurement {
	...
	public void addValue(int value) {//FEHLER!!!!
		...
		for( int i = 0; i < data.length; i++){//durchläuft das gesmte Array. nicht length+1, da ein arry bei 0 beginnt und bei .length endet
			temp[i] = data[i];// weist temp die "alten" werte von data zu. [i] stellt hierbei eine Variable dar, die alle Werte von 0 bis ende array annehmen kann.
			temp[temp.length-1] = value; //letzter Wert (value) wird an das ende des array "gehängt". letzter wert ist [data.length] und nicht lenght+1.
		}
	...
	public int getMinimumValue() {
		...
		for(int i = 0; i < data.length; i++){//durchläuft das data-Array
			if(data[i] < min){//falls ein Wert des data-Arrays kleiner als der aktuell kleinste ist
				...
				min = data[i];//wird er hier ersetzt
			}
		}
	...
	public int[] getValuesAboveThreshold(int threshold) { //threshold ist eine int-Variable, die den Schwellenwert darstellt.
		int[] temp =new int [0];//ein übergangs-array wird erstellt, dieses hat die Länge 0
		for(int i = 0; i < data.length;i++){//for-Schleife durchläuft das data-Array
			
			if(data[i] > threshold){//falls Werte aus dem data-Array größer sind als der threshold
				int[] temp2 = new int [temp.length+1];//erstellt ein neues Array mit der Bez. "temp2", dieses hat die Länge von "temp" plus 1.
				...
				for(int j = 0; j < temp.length; j++){//for-Schleife durchläuft das temp-Array,
					temp2[j] = temp[j];//temp2 werden die Werte von temp übergeben(hier fhlt noch was)
					
				}
				temp = temp2;//tmp bekommt die Werte von temp2 zugewiesen
			}
		}
