...
public class Measurement {
	...
	public void addValue(int value) {
		int [] yasin= new int[data.length+1];
		int i=0;
		for (i=0; i<data.length;i++) {yasin[i]=data[i];}
		yasin[i]=value;
	}
	...
	public void addValues(int[] values) {
		...
		for (int i=data.length; i< data.length+values.length; i++) {yasin[i]=values[i];}
	}
	...
	public int[] getValuesAboveThreshold(int threshold) {
		...
		for (int i=0; i<data.length; i++) {  yasin2[i]=yasin[j];};
