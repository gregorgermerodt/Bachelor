...
public class Matrix {
	...
	public void add(Matrix m) {
		for (int i=0; i<values.length;i++) {  
			for (int k=0; k<values.length; k++) {this.values[i][k]=this.values [i] [k]+ m.values[i] [k];}
		}
	...
	public void transpose() {
	    ...
	    int [] [] yasin = new int [this.values[0].length][this.values.length];
	for (int i=0; i<this.values.length;i++) 
{ for(int k=0; k<this.values[0].length; k++) {yasin [k][i]= this.values[i][k];} this.values= yasin; }
