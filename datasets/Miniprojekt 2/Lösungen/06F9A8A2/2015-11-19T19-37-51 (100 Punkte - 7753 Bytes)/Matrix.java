...
public class Matrix {
	...
	public void scalarMultiplication(int c) {
		for(int i = 0; i < values.length; i++) { // wie funktioniert diese Schachtelung?
	...
	public boolean isEqualTo(Matrix m) {
		if(values.length == m.values.length && values[0].length == m.values[0].length) {
			for(int i = 0; i < values.length; i++) {
				for(int j = 0; j < values[0].length; j++) { // was bedeutet values[0] bzw. values.length bei einem mehrdimensionalen bzw. zweidimensionalen Array?
