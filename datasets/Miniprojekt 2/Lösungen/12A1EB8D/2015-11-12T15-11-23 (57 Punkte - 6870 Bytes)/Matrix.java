...
public class Matrix {
	...
	public void add(Matrix m) {
		// laden wir die Werte in unsere Matrix.
		for (int i = 0; i < values.length; i++) {
			for (int j = 0; j < values[0].length; j++) {
				// deklarieren wir die Matrix
				// ersetzen wir unsere matrix mit m
	...
	public int[] getColumnVector(int col) {
		...
		// neues Array erstellt mit der länge von values(spalte)
		int[] loesung = new int[values.length];
		// laden wir die Werte von der
		for (int i = 0; i < values.length; i++) {
			// überprufen wir ob unsere J oder unsere Spalte = zu den gegeben in
			// der Matrix
			for (int j = 0; j < values[0].length; j++) {
				if (j == col) {
	...
	public boolean isEqualTo(Matrix m) {
		// wenn einer Flasch ist heißt das alle Falsch sind
