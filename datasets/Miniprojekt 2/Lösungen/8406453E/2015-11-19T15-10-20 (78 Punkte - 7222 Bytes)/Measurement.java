...
public class Measurement {
	...
	public void addValue(int value) {
		int[] neuerArray;
		neuerArray = new int[data.length + 1];
		for(int i = 0; i < data.length; i++){
			neuerArray[i] = data[i];
		
		} 
		neuerArray[neuerArray.length - 1] = value;
		data = neuerArray;
	...
	public void addValues(int[] values) {
		int[] neuerArray2 = new int[data.length + values.length];
		for(int index = 0; index < data.length; index++){
			neuerArray2[index] = data[index]; 
		}
		for(int index2 = data.length; index2 < data.length + values.length; index2++){
			neuerArray2[index2] = values[index2 - data.length];}
			data = neuerArray2;
		}
