...
public class Measurement {
	...
	public void addValues(int[] values) {
		...
		for (int j = 0, i = data.length; j < values.length; j++, i++){
					data3[i] = values[i];
				}
				data = data3;
		}
	...
	public int[] getValuesAboveThreshold(int threshold) {
		int anzahlFelder = 0;
	   	...
	   	if ( anzahlFelder == 0 || data[0] == 0){
	   		int [] data4 = new int [0];
	   		return data4;
	   	}
	   	else {
	   	int [] groFelder = new int [anzahlFelder];
	   	int temp = 0;
	   	for(int i = 0; i < data.length; i++){
	   		if (data[i] > threshold){
	   			groFelder[temp] = data[i];
	   			temp++;
	   		}
	   	}
	   	return groFelder;
