...
public class Matrix {
	...
	public void scalarMultiplication(int c) {
		...
		{
			for (int counter = 0; counter<=values[i].length -1; counter++)
			{
				values[i][counter] = values[i][counter] * c;
			}
		}
	}
	...
	public void add(Matrix m) {
		for(int i = 0; i<=values.length - 1;i++)
		{
			for (int counter = 0; counter<=values[i].length -1; counter++)
			{
				values[i][counter] = values[i][counter] + m.values[i][counter];
			}
		}
	}
	...
	public int[] getColumnVector(int col) {
		...
		for(int i = 0; i<=values.length - 1; i++)
		{
			columnVector[counter] = values[i][col]; 
			counter++;
		}
		return columnVector;
	}
