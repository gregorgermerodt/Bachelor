...
public class Matrix {
	...
	public void scalarMultiplication(int c) {
		...
		for (int row= 0; row < values.length;row++){
			for (int column = 0; column < values[row].length; column++){
				values[row][column] *=c;
				//values[row](indexZeile)[column](indexSpalte) = values[row][column]*c;   // Das selbe
			}
		}
	}
	...
	public boolean isEqualTo(Matrix m) {
		...
		if(values.length == m.values.length && values[0].length == m.values[0].length)
		{
			...
			{
				for( int indexSpalte = 0;indexSpalte < values[0].length;indexSpalte++) //values[0].length weil man auf die Zeile des Wertes 0 zugreift
				{
