...
public class Matrix {
	...
	public int[] getColumnVector(int col) {
		...
		for (int i = 0; i < values.length; i++) {
			vek[i] = this.values[i][col];
		}
		return vek;
	}

	public boolean isEqualTo(Matrix m) {
		if (m.values.length == values.length && m.values[0].length == values[0].length) { // Zeile und Spalte vergleichen ob gleich lang
			...
			if (zaehler == values.length * values[0].length) {
				return true;
			} else {
				return false;
			}
