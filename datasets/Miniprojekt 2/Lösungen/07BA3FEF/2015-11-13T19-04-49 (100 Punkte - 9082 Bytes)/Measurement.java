...
public class Measurement {
	...
	public void addValues(int[] values) {
		int[] t = new int [data.length+values.length];
	...
	public int getMinimumValue() {
		int minimum = data[0]; //int minimum= Interger.MAX_VALUE
		for (int i=0; i<data.length;i++)
			if(data[i]<minimum)
				minimum = data[i];
		return minimum;
	...
	public int[] getValuesAboveThreshold(int threshold) {
		...
		for(int i=0;i<data.length;i++)
		{
			if(data[i]>threshold)
			{
				 int [] k = new int[temp.length+1];
				 for (int j=0;j<temp.length;j++)
				 	{
					 k[j]=temp[j];
				 	}
				 k[temp.length]=data[i];
