...
public class Measurement {
  ...
  data=new int [0];
	...
	public void addValue(int value) {
		...
		neu[neu.length-1]=value;//oder[data.length]
		data=neu;
	}
...
for (int index=data.length; index < data.length + values.length;index++){
	//oder for(int index=0:index<values.length:index++){
...
data=neu;
	...
	public int getMinimumValue() {
		int min= Integer.MAX_VALUE;//oder int min=data[0];
