...
public class Matrix {
	...
	public boolean isEqualTo(Matrix m) {
		...
		for(int row=0; row<values.length;row++){
			for(int column=0; column<values[row].length;column++){
				...
				if(m.values[row].length!=values[row].length){
					probe++;
				}
			}
	...
	public void transpose() {
		int[][] newArr= new int[values[0].length][values.length];
		for(int row=0;row<newArr.length;row++){
			for(int column=0;column<newArr[row].length;column++){
				newArr[row][column]=values[column][row];
			}
		}
		values=newArr;
	}
