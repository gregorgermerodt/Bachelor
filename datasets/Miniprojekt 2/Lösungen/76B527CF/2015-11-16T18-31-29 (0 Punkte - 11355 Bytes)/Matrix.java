 ...
 *jeder Wert aij aus der einen Matrix mit aij aus der anderen Matrix übereinstimmt*/
	...
	public int[][] transpose() { 														// neues Array ersten dasdie Breite und länge von 
			int[][] neuMatrix;
			for (int row =0 ; row < values.length;row++){							// fragt die x- Koordinaten ab
				for(int column =0; column < values[row].length; column++){			// fragt die y- Koordinaten ab
																					
					neuMatrix[column][row] = neuMatrix [column][row];
				}
			}return	neuMatrix;
	}
